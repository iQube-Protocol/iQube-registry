...
export const IQubeTemplateSelector = ({ onSelectTemplate, onCreateCustom }: IQubeTemplateSelectorProps) => {
  return (
    <div className="space-y-6">
      <div className="text-center">
        <h2 className="text-2xl font-bold text-slate-900 mb-2">Choose an iQube Template</h2>
        <p className="text-slate-600">Start with a pre-designed template or create your own custom schema</p>
      </div>

      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {templates.map((template) => (
          <Card key={template.id} className="relative cursor-pointer hover:shadow-lg transition-shadow">
            {template.isPopular && (
